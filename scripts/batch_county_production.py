#!/usr/bin/env python3
"""
Production batch: 10 counties across varied themes for Etsy listing assets.
Uses finalized layout: bbox centering, ghost bg, PIL fonts, variant D text.
"""
...
CANVAS_W_IN = 24
CANVAS_H_IN = 36
CANVAS_ASPECT = CANVAS_W_IN / CANVAS_H_IN
FADE_START = 0.80

# 10 counties x varied themes — geographic diversity + shape variety
RENDERS = [
    ("Union", "NJ", "custom_3map/nordic_complex"),       # requested — small urban NJ
    ("Cook", "IL", "custom_3map/dark_teal"),              # Chicago — huge metro
    ("Marin", "CA", "custom_3map/sage_atlas"),            # coastal CA, interesting shape
    ("Davidson", "TN", "custom_3map/sky_blue"),           # Nashville
    ("Fulton", "GA", "custom_3map/noir"),                 # Atlanta — long narrow
    ("Travis", "TX", "custom_3map/teal_coral"),           # Austin
    ("Hennepin", "MN", "custom_3map/nordic_complex"),     # Minneapolis
    ("Allegheny", "PA", "custom_3map/dark_teal"),         # Pittsburgh — rivers
    ("Wake", "NC", "custom_3map/sage_atlas"),             # Raleigh
    ("Maui", "HI", "custom_3map/rose_blush"),             # island shape
]
...
def render_one(county: str, state: str, theme: str) -> tuple[str, float]:
    """Render a single county with ghost + PIL text."""
    ...
    # --- PIL text: variant D spacing, optically centered line ---
    draw = ImageDraw.Draw(composite)
    scale = min(CANVAS_W_IN, CANVAS_H_IN) / 12.0 * (DPI / 72.0) * 0.5
...
def main() -> None:
    ...
    for i, (county, state, theme) in enumerate(RENDERS, 1):
        ...
        print(f"\n[{i}/{len(RENDERS)}] {county} County, {state} -- {theme_name}")
        ...
        try:
            ...
            print(f"  OK: {elapsed:.0f}s, {file_mb:.1f}MB")
