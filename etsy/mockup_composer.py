...
# ── Lifestyle mockups — for print listings only ──────────────────────────────

LIFESTYLE_MOCKUPS: list[MockupDef] = [
    # Single frame, green velvet couch, wood frame
    MockupDef(
        filename=str(LIFESTYLE_DIR / "Frame 15.psd"),
        short_name="frame15",
        render_size="24x36",
        slots=[],
    ),
    # Single frame, minimalist shelf, black frame
    MockupDef(
        filename=str(LIFESTYLE_DIR / "Frame 32.psd"),
        short_name="frame32",
        render_size="24x36",
        slots=[],
    ),
    # NOTE: PSD4 (4-frame grid) is NOT generated here as a standard mockup.
    # For color-variant styles (Blueprint, MonoMap), a labeled PSD4 with
    # color names + title is generated by create_labeled_psd4() in batch_universal.py.
    # For non-color styles, the PSD4 template is available at BEST_DIR / "PSD.psd"
    # but is not auto-generated to avoid unlabeled duplicates.
]

# ── Extended lifestyle mockups — additional 40Vintage frames ─────────────────

EXTENDED_LIFESTYLE_MOCKUPS: list[MockupDef] = [
    # Tier 1 — best lifestyle scenes
    # Gold frame hung on concrete wall — perfectly flat, unique texture
    MockupDef(
    ...
    # Gold frame + fiddle leaf fig — premium lifestyle
    MockupDef(
        filename=str(LIFESTYLE_DIR / "Frame 25.psd"),
        short_name="frame25",
        render_size="24x36",
        slots=[],
    ),
    # Oak hung on wall + boho shelf — beautiful staging
    MockupDef(
        filename=str(LIFESTYLE_DIR / "Frame 33.psd"),
        short_name="frame33",
        render_size="24x36",
        slots=[],
    ),
    # Black frame wall-hung + plant — modern lifestyle
    MockupDef(
        filename=str(LIFESTYLE_DIR / "Frame 40.psd"),
        short_name="frame40",
        render_size="24x36",
        slots=[],
    ),
    # Wood frame, boho plants room — warm, inviting
    MockupDef(
        filename=str(LIFESTYLE_DIR / "Frame 21.psd"),
        short_name="frame21",
        render_size="24x36",
        slots=[],
    ),
    # Tier 2 — more variety
    # Gold frame + dried flowers vase — warm, gift aesthetic
    MockupDef(
        filename=str(LIFESTYLE_DIR / "Frame 8.psd"),
        short_name="frame8",
        render_size="24x36",
        slots=[],
    ),
    # Bronze/copper frame on table — minimal, elegant
    MockupDef(
        filename=str(LIFESTYLE_DIR / "Frame 28.psd"),
        short_name="frame28",
        render_size="24x36",
        slots=[],
    ),
    # Oak frame with mat, herringbone floor — clean and versatile
    MockupDef(
        filename=str(LIFESTYLE_DIR / "Frame 30.psd"),
        short_name="frame30",
        render_size="24x36",
        slots=[],
    ),
    # Gold frame, Parisian room + plants — premium feel
    MockupDef(
        filename=str(LIFESTYLE_DIR / "Frame 43.psd"),
        short_name="frame43",
        render_size="24x36",
        slots=[],
    ),
    # Black frame, herringbone floor — modern, clean
    MockupDef(
        filename=str(LIFESTYLE_DIR / "Frame 11.psd"),
        short_name="frame11",
        render_size="24x36",
        slots=[],
    ),
    # Gold frame + mat, leaning, herringbone — clean look
    MockupDef(
        filename=str(LIFESTYLE_DIR / "Frame 44.psd"),
        short_name="frame44",
        render_size="24x36",
        slots=[],
    ),
...
# ── Extended flat mockups — unused Best/ PSDs ────────────────────────────────

EXTENDED_FLAT_MOCKUPS: list[MockupDef] = [
    MockupDef(
        filename=str(BEST_DIR / "Mockup56.psd"),
        short_name="mockup56",
        render_size="24x36",
        slots=[],
    ),
    MockupDef(
        filename=str(BEST_DIR / "Mockup83.psd"),
        short_name="mockup83",
        render_size="24x36",
        slots=[],
    ),
    MockupDef(
        filename=str(BEST_DIR / "Mockup94.psd"),
        short_name="mockup94",
        render_size="24x36",
        slots=[],
    ),
    # 2 frames, side-by-side — different layout from existing 2Frames/FramePSD
    MockupDef(
        filename=str(BEST_DIR / "WH-2V_Post.psd"),
        short_name="wh2v",
        render_size="24x36",
        slots=[],
        featured_slot=0,
...
# ── Extended multi-frame from PSD Mockups/ ───────────────────────────────────

PSD_MOCKUPS_DIR = Path(r"C:\MapGen_GLcollective\etsy\TUR2\PSD Mockups")

EXTENDED_MULTI_MOCKUPS: list[MockupDef] = [
    # 2 slots: large + small frame — asymmetric layout
    MockupDef(
        filename=str(PSD_MOCKUPS_DIR / "Mockup13.psd"),
        short_name="mockup13",
        render_size="24x36",
        slots=[],
        featured_slot=0,  # featured in the larger frame
    ),
]
...
# Flat mockups — used for all listings (digital + print)
ALL_MOCKUPS: list[MockupDef] = AUTO_MOCKUPS + MANUAL_MOCKUPS

# Print mockups — flat + lifestyle (for physical print listings)
PRINT_MOCKUPS: list[MockupDef] = AUTO_MOCKUPS + MANUAL_MOCKUPS + LIFESTYLE_MOCKUPS

# Extended set — all available mockups including new additions
ALL_LIFESTYLE: list[MockupDef] = LIFESTYLE_MOCKUPS + EXTENDED_LIFESTYLE_MOCKUPS
ALL_FLAT_EXTENDED: list[MockupDef] = AUTO_MOCKUPS + MANUAL_MOCKUPS + EXTENDED_FLAT_MOCKUPS
ALL_EXTENDED: list[MockupDef] = (
    AUTO_MOCKUPS + MANUAL_MOCKUPS + EXTENDED_FLAT_MOCKUPS
    + LIFESTYLE_MOCKUPS + EXTENDED_LIFESTYLE_MOCKUPS
    + EXTENDED_MULTI_MOCKUPS
)
...
def find_city_render(city_slug: str, render_size: str) -> Path | None:
    ...
    Searches renders/ and renders/POSTED/ subdirectories.
    """
    # Direct path (in-progress or style-suffixed renders)
    render_path = RENDER_DIR / city_slug / f"{city_slug}_{render_size}.png"
    if render_path.exists():
        return render_path
    # Classic posted renders (under POSTED/)
    posted_path = RENDER_DIR / "POSTED" / "DefaultMap_Posted" / city_slug / f"{city_slug}_{render_size}.png"
    if posted_path.exists():
        return posted_path
    # Legacy flat path (backward compat)
    legacy_path = RENDER_DIR / "DefaultMap_Posted" / city_slug / f"{city_slug}_{render_size}.png"
    if legacy_path.exists():
        return legacy_path
    # Florence posted renders (under POSTED/)
    florence_path = RENDER_DIR / "POSTED" / "FlorenceMap_Posted" / f"{city_slug}_florence" / f"{city_slug}_{render_size}.png"
    if florence_path.exists():
        return florence_path
    # Legacy Florence path
    florence_legacy = RENDER_DIR / "FlorenceMap_Posted" / f"{city_slug}_florence" / f"{city_slug}_{render_size}.png"
    if florence_legacy.exists():
        return florence_legacy
    return None
...
def compose_mockup(
    ...
    """
    # Resolve PSD path — absolute if it exists, otherwise relative to MOCKUP_DIR
...
def list_mockups() -> None:
    ...
    for m in ALL_MOCKUPS + LIFESTYLE_MOCKUPS:
        psd_path = Path(m.filename) if Path(m.filename).is_absolute() else MOCKUP_DIR / m.filename
